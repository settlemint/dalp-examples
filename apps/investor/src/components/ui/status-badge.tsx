...
/**
 * Domain status values that surface across the investor portal — KYC review
 * states (from `user.kyc.profile.read().latestStatus`) and async-transaction
 * states (from `transaction.read().status`). Mapped to a coherent colour + dot
 * so the same status always reads the same way wherever it appears.
 */
